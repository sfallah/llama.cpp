...
// Rounding policy for the aspect-preserving pad path of img_tool::resize.
//   PAD_ROUNDING_CEIL    — ceil the scaled dims, floor the centering offset (default;
//                          matches the historical behaviour of this function).
//   PAD_ROUNDING_NEAREST — round the scaled dims, round the centering offset.
//                          Matches HF / Pillow byte-for-byte (Pillow rounds rather
//                          than ceils, and centers with Python round, not floor).
enum pad_rounding {
    PAD_ROUNDING_CEIL,
...
struct img_tool {
    static void resize(
        ...
        if (!add_padding) {
            ...
            auto round_dim = [rounding](float x) {
                return rounding == PAD_ROUNDING_NEAREST
                    ? static_cast<int>(std::round(x))
                    : static_cast<int>(std::ceil(x));
            };
            int new_width  = std::min(round_dim(src.nx * scale), target_resolution.width);
            int new_height = std::min(round_dim(src.ny * scale), target_resolution.height);
